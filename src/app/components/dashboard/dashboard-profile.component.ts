...
import { loadMyProfile, deleteAccount, deleteExperience, deleteEducation } from '../../store/profile/profile.actions';
...
@Component({
    ...
    <div class="subpage">
      ...
      <div class="subpage-header">
        <div>
          ...
          <p class="subpage-sub">Manage your profile, experience and education</p>
        </div>
        <div class="header-actions">
          <a routerLink="/edit-profile" class="btn-outline">✏️ Edit Profile</a>
        </div>
      </div>
      ...
      <ng-container *ngIf="!(loading$ | async)">
        <ng-container *ngIf="profile$ | async as profile; else noProfile">
          ...
          <div class="info-card">
            <div class="card-header-row">
              ...
              <a routerLink="/edit-profile" class="btn-sm">Edit</a>
            </div>

            <div class="profile-hero">
              <img *ngIf="(currentUser$ | async)?.avatar"
                   [src]="(currentUser$ | async)?.avatar"
                   [alt]="(currentUser$ | async)?.name"
                   class="profile-avatar"/>
              <div *ngIf="!(currentUser$ | async)?.avatar" class="profile-avatar-placeholder">
                {{ (currentUser$ | async)?.name?.charAt(0)?.toUpperCase() }}
              </div>
              <div class="profile-hero-info">
                <p class="hero-name">{{ (currentUser$ | async)?.name }}</p>
                <p class="hero-handle" *ngIf="profile.handle">&#64;{{ profile.handle }}</p>
                <p class="hero-status" *ngIf="profile.status">{{ profile.status }}</p>
              </div>
            </div>
          ...
          <div class="info-card">
            <div class="card-header-row">
              ...
              <a routerLink="/add-experience" class="btn-sm">+ Add</a>
            </div>

            <div class="empty-section" *ngIf="!profile.experience?.length">
              <p>No experience added yet.</p>
              <a routerLink="/add-experience" class="btn-outline-sm">Add Experience →</a>
            </div>

            <div class="entry-list" *ngIf="profile.experience?.length">
              <div class="entry-item" *ngFor="let exp of profile.experience">
                ...
                <button class="btn-remove" (click)="onDeleteExp(exp._id!)">Remove</button>
              </div>
            </div>
          </div>
          ...
          <div class="info-card">
            <div class="card-header-row">
              ...
              <a routerLink="/add-education" class="btn-sm">+ Add</a>
            </div>

            <div class="empty-section" *ngIf="!profile.education?.length">
              <p>No education added yet.</p>
              <a routerLink="/add-education" class="btn-outline-sm">Add Education →</a>
            </div>

            <div class="entry-list" *ngIf="profile.education?.length">
              <div class="entry-item" *ngFor="let edu of profile.education">
                ...
                <button class="btn-remove" (click)="onDeleteEdu(edu._id!)">Remove</button>
              </div>
            </div>
          </div>
          ...
          <div class="danger-zone">
            ...
            <p class="danger-sub">Once you delete your account, all data will be permanently removed.</p>
            <button class="btn-danger" (click)="onDeleteAccount()">🗑 Delete My Account</button>
          </div>
        ...
        <ng-template #noProfile>
          <div class="no-profile-card">
            ...
            <p>Set up your developer profile to get discovered.</p>
            <a routerLink="/create-profile" class="btn-primary">Create Profile →</a>
          </div>
        </ng-template>
      </ng-container>
    </div>
  ...
  styles: [`
    ...
    .btn-outline { padding: 0.6rem 1.25rem; background: rgba(99,102,241,0.1); border: 1px solid rgba(99,102,241,0.3); color: #a78bfa; border-radius: 10px; text-decoration: none; font-weight: 700; font-size: 0.875rem; transition: all 0.15s; white-space: nowrap; }
    ...
    .btn-sm { padding: 0.35rem 0.9rem; background: rgba(99,102,241,0.1); border: 1px solid rgba(99,102,241,0.2); color: #a78bfa; border-radius: 7px; text-decoration: none; font-size: 0.8rem; font-weight: 600; transition: all 0.15s; }
    ...
    /* Entry list (exp/edu) */
    .empty-section { color: #475569; font-size: 0.875rem; display: flex; flex-direction: column; align-items: flex-start; gap: 0.75rem; }
    .btn-outline-sm { padding: 0.4rem 0.9rem; background: rgba(99,102,241,0.08); border: 1px solid rgba(99,102,241,0.2); color: #a78bfa; border-radius: 7px; text-decoration: none; font-size: 0.8rem; font-weight: 600; transition: all 0.15s; }
    .btn-outline-sm:hover { background: rgba(99,102,241,0.16); }
    ...
    .btn-remove { padding: 0.3rem 0.7rem; background: rgba(239,68,68,0.08); border: 1px solid rgba(239,68,68,0.2); color: #f87171; border-radius: 6px; cursor: pointer; font-size: 0.78rem; font-weight: 600; transition: all 0.15s; flex-shrink: 0; white-space: nowrap; }
    ...
    /* Danger Zone */
    .danger-zone { background: rgba(239,68,68,0.05); border: 1px solid rgba(239,68,68,0.15); border-radius: 14px; padding: 1.5rem 1.75rem; margin-bottom: 1.25rem; }
    .danger-title { color: #f87171; font-weight: 700; font-size: 0.95rem; margin: 0 0 0.4rem; }
    .danger-sub { color: #64748b; font-size: 0.85rem; margin: 0 0 1rem; }
    .btn-danger { padding: 0.6rem 1.5rem; background: rgba(239,68,68,0.1); border: 1px solid rgba(239,68,68,0.3); color: #f87171; border-radius: 8px; cursor: pointer; font-weight: 700; transition: all 0.2s; }
    ...
    .btn-primary { display: inline-block; padding: 0.75rem 2rem; background: linear-gradient(135deg,#6366f1,#8b5cf6); color: white; border-radius: 10px; text-decoration: none; font-weight: 700; transition: all 0.15s; }
